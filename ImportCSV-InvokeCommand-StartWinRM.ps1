...
$path = Write-Host "Enter full path and filename of csv (needs to be csv of one colum, with ComputerName as column header):"
$workstations = Import-CSV $path
foreach ($i in $workstations)
    {
        $wsname = $($i.ComputerName)
        Write-Host " "
        Write-Host "=============================================================================="
        Write-Host "Checking:" $wsname
        Write-Host " "
        Invoke-Command -ComputerName $wsname -ScriptBlock {net start winrm}
        Write-Host "=============================================================================="
        Write-Host " "
    }
